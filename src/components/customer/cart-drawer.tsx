          ...
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div
                ...
                className="flex gap-3 p-3 bg-white rounded-xl border border-[#E2E8F0]"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.variantLabel && (
                    <p className="text-xs text-[#64748B] mt-0.5">{item.variantLabel}</p>
                  )}
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <div className="flex items-center gap-0.5 bg-[#F1F5F9] rounded-lg p-0.5">
                      ...
                      <span className="w-7 text-center text-sm font-medium text-[#0F172A]">
                        {item.qty}
                      </span>
                      <button
                        onClick={() => updateQty(item.productId, item.variantId, item.qty + 1)}
        ...
        {items.length > 0 && (
          <div className="border-t border-[#E2E8F0] px-4 py-3.5 space-y-3">
            ...
            <Link
              href="/keranjang"
              onClick={onClose}
              className="block w-full text-center bg-[#0F172A] hover:bg-[#1E293B] text-white font-semibold py-3 rounded-xl transition-colors text-sm"
            >
              Lihat Keranjang
            </Link>
            <Link
              href="/checkout"
              onClick={onClose}
              className="block w-full text-center border border-[#E2E8F0] text-[#0F172A] font-medium py-3 rounded-xl hover:bg-[#F1F5F9] transition-colors text-sm"
            >
              Checkout
            </Link>
          </div>
