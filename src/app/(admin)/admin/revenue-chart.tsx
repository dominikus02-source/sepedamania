  ...
  return (
    <ResponsiveContainer width="100%" height={256}>
      <AreaChart data={safeData}>
        ...
        <YAxis tick={{ fontSize: 12, fill: '#8E8E93' }} tickFormatter={formatRevenueTick} axisLine={{ stroke: '#E5E5EA' }} />
        <Tooltip formatter={(val) => [formatPrice(Number(val)), 'Pendapatan']} labelFormatter={(label) => `Tanggal: ${label}`} />
        <Area type="monotone" dataKey="revenue" stroke="#F5A623" fill="url(#revenueGradient)" strokeWidth={2} />
      </AreaChart>
    </ResponsiveContainer>
  );
}
